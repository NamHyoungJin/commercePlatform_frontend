...
## 1. 원칙
- **프론트의 시스템은 동적 빌드 안 함(X)·변경 불가, 정적 빌드만 진행(O)·변경 불가** (목표 규칙)
- **동적 구분은 쿼리 파라미터로**
...
- **목표 아키텍처**에서는 `next.config`에 **`output: 'export'`**(정적 HTML 내보내기)를 둔다.
- **이 레포(`frontend-www`) 현황**: `next.config.ts`에는 **`output: 'export'`가 아직 없다.** (전환 절차·동적 라우트는 §7)
...
- 정적 export를 켠 뒤 Middleware가 꼭 필요해지면 **`output: 'export'` 제거**와 배포 방식 변경이 먼저 결정되어야 하며, 그 전에는 미들웨어만 도입하지 말 것.
...
| `/customer/notice` | `page`, `q`, `id` | 고객센터 공지 목록·검색·상세 (`?id=110` = 상세, 목록은 `id` 없음) |
| `/customer/score` | `page`, `q`, `id` | 악보신청 게시판 (동일) |
| `/customer/faq` | `page`, `q`, `id` | FAQ (동일) |
| `/customer/qna` | `page`, `q`, `id` | Q&A (동일) |
