...
export type ScoresClientProps = {
  /** 페이지네이션·필터 URL 기본 경로 (`/scores` 또는 `/scoresSearch`) */
  listBasePath?: string;
};

export default function ScoresClient({ listBasePath = '/scores' }: ScoresClientProps) {
  ...
  const isScoresSearchRoute = listBasePath === '/scoresSearch';
  const searchQueryRaw = (searchParams.get('q') ?? '').trim();
  const musicSidRaw = (searchParams.get('music_sid') ?? '').trim();
  const musicSearchSubtitle = useMemo(() => {
    if (!musicSidRaw) return '';
    if (loading) return '동일 곡(music)에 연결된 악보를 모두 표시합니다.';
    const name = scores[0]?.music?.kor_name?.trim();
    return name
      ? `「${name}」에 연결된 악보를 모두 표시합니다.`
      : '동일 곡(music)에 연결된 악보를 모두 표시합니다.';
  }, [musicSidRaw, scores, loading]);
  const searchSubtitleMessage = musicSidRaw
    ? musicSearchSubtitle
    : searchQueryRaw ||
      '검색어를 입력해 보세요. 아래 검색창에서 곡명·작곡가·아티스트를 입력할 수 있습니다.';
  ...
  const hero = useMemo(() => {
    ...
    return {
      title: '악보 카탈로그',
      subtitle: '필터와 검색으로 원하는 악보를 찾으세요.',
    };
  }, [sort]);
  ...
  useEffect(() => {
    ...
    const params: ScoreListParams = {
      ...
      music_sid: musicSidRaw || undefined,
  ...
  const goPage = (p: number) => {
    ...
    router.push(`${listBasePath}?${pStr.toString()}`);
  };
  ...
  const titleBlock = (
    <div className="min-w-0 shrink-0 lg:max-w-xl">
      {isScoresSearchRoute ? (
        chartListStyle ? (
          <>
            <h1 className="home-album-strip-title popular-chart-section-title">검색 (search)</h1>
            <p className="home-album-strip-sub popular-chart-section-sub mt-3 max-w-4xl text-balance break-words">
              {searchSubtitleMessage}
            </p>
          </>
        ) : (
          <>
            <h1 className="text-3xl font-bold tracking-tight text-text-primary md:text-4xl">검색 (search)</h1>
            <p className="home-album-strip-sub mt-3 max-w-2xl text-balance break-words">{searchSubtitleMessage}</p>
          </>
        )
      ) : chartListStyle ? (
        <>
  ...
  const filterBlock = (
    <div className="w-full min-w-0 shrink-0 lg:max-w-md xl:max-w-lg">
      <div className={filterCardClass}>
        {filters ? (
          <ScoreFilter filters={filters} basePath={listBasePath} />
        ) : (
  ...
  const chartToolbarFilters = (
    <div className="mb-8 rounded-xl border border-[var(--border-subtle)] bg-surface/90 p-4 shadow-sm backdrop-blur-sm sm:p-5 md:mb-10">
      {filters ? (
        <ScoreFilter filters={filters} layout="toolbar" hideSearch basePath={listBasePath} />
      ) : (
  ...
  const chartHeroTitle = (
    <div className="mx-auto max-w-4xl text-center">
      {isScoresSearchRoute ? (
        <>
          <h1 className="home-album-strip-title popular-chart-section-title">검색 (search)</h1>
          <p className="home-album-strip-sub popular-chart-section-sub mx-auto mt-3 max-w-3xl text-balance break-words">
            {searchSubtitleMessage}
          </p>
        </>
      ) : (
  ...
  if (chartListStyle) {
    const heroImageSrc =
      sort === 'popular' ? '/img/_common/flute-1427650.jpg' : '/img/_common/whoOnlyonemusic_1.png';
    const heroImageClass =
      sort === 'popular' ? 'object-cover object-center' : 'object-cover object-center';
