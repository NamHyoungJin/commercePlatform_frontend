...
export default function ScoresCategoryClient() {
  const searchParams = useSearchParams();
  const router = useRouter();

  const rawCategory = searchParams.get('code') ?? '';
  const validCategory = SCORE_CATEGORY_CODES.includes(rawCategory as (typeof SCORE_CATEGORY_CODES)[number])
    ? rawCategory
    : '';
  ...
  const categoryListPath = '/score/ScoreCategoryList';
  const filterBasePath = validCategory
    ? `${categoryListPath}?code=${encodeURIComponent(validCategory)}`
    : categoryListPath;

  const hero = useMemo(() => {
    const catHeading = validCategory ? `${scoreCategoryLabel(validCategory)} 카테고리` : '카테고리';
    if (sort === 'popular') {
      return {
        title: `${catHeading} · 인기`,
        subtitle: '이 카테고리에서 많이 찾는 악보입니다.',
      };
    }
    if (sort === 'newest') {
      return {
        title: `${catHeading} · 최신`,
        subtitle: '최근 등록된 악보입니다.',
      };
    }
    return {
      title: catHeading,
      subtitle: '카테고리별 악보를 찾아보세요.',
    };
  }, [sort, validCategory]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const listParams: ScoreListParams = {
  ...
  }, [searchParams, validCategory, page]);
  ...
  const goPage = (p: number) => {
    ...
    if (validCategory) pStr.set('code', validCategory);
    if (nextPage <= 1) pStr.delete('page');
    else pStr.set('page', String(nextPage));
    router.push(`${categoryListPath}?${pStr.toString()}`);
  };
  ...
  const chartHeroHeading = validCategory
    ? `${scoreCategoryLabel(validCategory)} 카테고리`
    : '카테고리';

  const chartHeroTitle = (
    <div className="mx-auto max-w-4xl text-center">
      <h1 className="home-album-strip-title popular-chart-section-title">{chartHeroHeading}</h1>
      <nav
        className="mx-auto mt-5 flex max-w-2xl flex-wrap items-center justify-center gap-2 sm:mt-6 sm:gap-3"
        aria-label="카테고리 바로가기"
      >
        {SCORE_CATEGORY_MENU.map(({ code, label }) => {
          const active = validCategory === code;
          return (
            <Link
              key={code}
              href={scoreCategoryHref(code)}
              className={`rounded-full border px-4 py-2 text-sm font-medium transition-colors ${
                active
                  ? 'border-white bg-white text-neutral-900'
                  : 'border-white/25 bg-white/10 text-white/90 hover:border-white/45 hover:bg-white/18'
              }`}
            >
              {label}
            </Link>
  ...
  if (chartListStyle) {
    const heroImageSrc = '/img/_common/whoOnlyonemusic_3.png';
